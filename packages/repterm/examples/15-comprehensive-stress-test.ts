...
describe('Interactive workflows', { record: true }, () => {
  test('conditional, accumulation, hidden input, menu, CSV parsing', async ({ terminal }) => {
    await step('interactive workflows', { typingSpeed: 0 }, async () => {
      ...
      '`;
      const cond = terminal.run(condScript, { interactive: true });
      ...
      '`;
      const acc = terminal.run(accScript, { interactive: true });
      ...
      const pw = terminal.run(pwScript, { interactive: true });
      ...
      '`;
      const menu = terminal.run(menuScript, { interactive: true });
      ...
      const csv = terminal.run(csvScript, { interactive: true });
...
describe('Shell constructs', { record: true }, () => {
  test('subshells, pipelines, heredocs, arrays, conditionals, loops', async ({ terminal }) => {
    await step('shell constructs', { typingSpeed: 0 }, async () => {
      // Nested subshells
      const sub = await terminal.run('(echo outer; (echo inner; (exit 7)))');
      ...
      const arr = await terminal.run(
        'bash -c \'arr=(alpha beta gamma delta); echo "len=${#arr[@]} first=${arr[0]} last=${arr[-1]}"; str="Hello World"; echo "upper=${str^^} len=${#str}"\'',
      );
      ...
      // Conditional chains
      const chain1 = await terminal.run('true && echo "A" && false || echo "B"');
      expect(chain1).toSucceed();
      expect(chain1).toContainInOutput('A');
      expect(chain1).toContainInOutput('B');
      const chain2 = await terminal.run('false && echo "SKIP" || echo "FALLBACK"');
      ...
      const nest = await terminal.run('echo "result: $(echo "inner: $(echo deep)")"');
      expect(nest).toSucceed();
      expect(nest).toContainInOutput('deep');

      // While loop
      const loop = await terminal.run(
        'i=0; while [ $i -lt 5 ]; do echo "iter:$i"; i=$((i+1)); done; echo "done:$i"',
      );
      ...
      // Background jobs
      const bg = await terminal.run('echo "JOB1" & echo "JOB2" & echo "JOB3"; wait');
      expect(bg).toSucceed();
...
describe('Output handling', { record: true }, () => {
  test('volume, interleaving, ANSI, special chars, stderr', async ({ terminal }) => {
    await step('output handling', { typingSpeed: 0 }, async () => {
      // Large volume (500 lines)
      const vol = await terminal.run('seq 1 500');
      ...
      // ANSI color codes
      const ansi = await terminal.run(
        'printf "\\033[1;31mBOLD_RED\\033[0m \\033[4;32mUNDERLINE_GREEN\\033[0m \\033[7mINVERTED\\033[0m\\n"',
      );
      ...
      // Special characters
      const special = await terminal.run(
        'printf "TAB:\\there\\nQUOTE:\\"hello\\"\\nBACKSLASH:\\\\end\\n"',
      );
      ...
      const blanks = await terminal.run(
        'echo "first"; echo; echo; echo "after blanks"; echo; echo "end"',
      );
      ...
      const longLine = await terminal.run(
        'python3 -c "print(\'A\' * 500)" 2>/dev/null || printf "%0.sA" $(seq 1 500)',
      );
      expect(longLine).toSucceed();
      console.log('  500-char line captured');

      // Control characters
      const ctrl = await terminal.run(
        'printf "normal\\x07BELL\\x08BS\\x1b[33myellow\\x1b[0m end\\n"',
      );
      ...
      const errOnly = await terminal.run('echo "only-error" >&2');
      expect(errOnly).toSucceed();
      expect(errOnly).toContainInOutput('only-error');

      // Rapid sequential echo
      const rapid = await terminal.run(
        'for i in $(seq 1 50); do echo "rapid:$i"; done',
      );
      ...
      // Long argument list
      const longArgs = await terminal.run('echo $(seq 1 100 | tr "\\n" " ")');
...
describe('Signals and process lifecycle', { record: true }, () => {
  test('EXIT trap, SIGTERM, then SIGINT and loop interrupt', async ({ terminal }) => {
    await step('signals', { typingSpeed: 0 }, async () => {
      // Non-interactive first (before any interrupt disturbs shell state)

      // EXIT trap
      const exitTrap = await terminal.run(
        'bash -c \'trap "echo EXIT_TRAP" EXIT; echo "before"; exit 0\'',
      );
      ...
      // SIGTERM self-kill
      const term = await terminal.run('bash -c \'kill -TERM $$\'');
      expect(term).toFail();
      ...
      const trapped = terminal.run(
        'bash -c \'trap "echo INTERRUPTED; exit 42" INT; echo "READY"; sleep 999\'',
        { interactive: true },
      );
      ...
      const plain = terminal.run('sleep 999', { interactive: true });
      await plain.start();
      ...
      const loop = terminal.run(
        'bash -c \'echo "START"; while true; do sleep 0.1; done\'',
        { interactive: true },
      );
...
describe('Exit codes', { record: true }, () => {
  test('sequences, boundaries, isolation, alternating, pipelines', async ({ terminal }) => {
    await step('exit codes', { typingSpeed: 0 }, async () => {
      // Exit 0 through 5 (subshell to avoid killing the interactive shell)
      const codes: number[] = [];
      for (let i = 0; i <= 5; i++) {
        const r = await terminal.run(`(exit ${i})`);
        codes.push(r.code);
      ...
      const r255 = await terminal.run('(exit 255)');
      expect(r255).toHaveExitCode(255);
      const r256 = await terminal.run('(exit 256)');
      expect(r256).toHaveExitCode(0);
      console.log(`  exit 255 → ${r255.code}, exit 256 → ${r256.code}`);

      // Command not found
      const notFound = await terminal.run('__nonexistent_xyz__ 2>/dev/null');
      expect(notFound).toFail();
      console.log(`  not found → code=${notFound.code}`);

      // Isolation: failure does not affect next command
      const fail = await terminal.run('(exit 99)');
      expect(fail).toFail();
      const ok = await terminal.run('echo "clean slate"');
      expect(ok).toSucceed();
      ...
      for (let i = 0; i < 10; i++) {
        const r = await terminal.run(`(exit ${i % 2})`);
        results.push(r.code);
      }
      ...
      const pipeOk = await terminal.run('false | echo "piped"');
      expect(pipeOk).toSucceed();
      const pipeFail = await terminal.run('echo "hello" | grep nonexistent');
...
describe('Environment, metadata, and assertions', { record: true }, () => {
  test('env, cwd, duration, snapshots, text matching, delayed output', async ({ terminal }) => {
    await step('env and metadata', { typingSpeed: 0 }, async () => {
      ...
      const env = await terminal.run(
        'CUSTOM_VAR=repterm_test_42 bash -c \'echo "val=$CUSTOM_VAR"\'',
      );
      ...
      const cwd = await terminal.run('cd /tmp && pwd');
      expect(cwd).toSucceed();
      expect(cwd).toContainInOutput('/tmp');

      // Env + cwd combined
      const combo = await terminal.run(
        'cd /tmp && TAG=MARKER bash -c \'echo "$TAG $(pwd)"\'',
      );
      ...
      const tracked = await terminal.run(cmd);
      expect(tracked.command).toBe(cmd);

      // Duration: slow command
      const slow = await terminal.run('sleep 0.2 && echo "timed"');
      ...
      const quick = await terminal.run('echo "instant"');
      expect(quick).toSucceed();
      expect(quick.duration).toBeLessThan(3000);

      // Snapshot captures accumulated state
      await terminal.run('echo "MARKER_ALPHA"');
      await terminal.run('echo "MARKER_BETA"');
      await terminal.run('echo "MARKER_GAMMA"');
      ...
      // toContainText
      await terminal.run('echo "unique-sentinel-xyzzy"');
      await expect(terminal).toContainText('unique-sentinel-xyzzy');

      // toMatchPattern
      await terminal.run('echo "Version: 3.14.159"');
      await expect(terminal).toMatchPattern(/Version: \d+\.\d+\.\d+/);

      // Negative assertions
      await terminal.run('echo "all good"');
      ...
      const delayed = terminal.run('sleep 0.3 && echo "DELAYED_MARKER"', {
        interactive: true,
      });
      await delayed.start();
...
describe('Mixed workflows', { record: true }, () => {
  test('interactive + non-interactive, recovery, pipeline, file ops', async ({ terminal }) => {
    await step('mixed workflows', { typingSpeed: 0 }, async () => {
      ...
      const proc = terminal.run(
        'bash -c \'read -p "Input: " val; echo "Echo: $val"\'',
        { interactive: true },
      );
      ...
      await proc;
      const r2 = await terminal.run('echo "non-interactive-check"');
      expect(r2).toSucceed();
      const snap = await terminal.snapshot();
      ...
      // Error recovery
      const fail = await terminal.run('false');
      expect(fail).toFail();
      const recovered = await terminal.run('echo "RECOVERED"');
      ...
      // Long pipeline
      const pipeline = await terminal.run(
        'seq 1 100 | grep -E "^[0-9]{2}$" | sort -n | tail -5',
      );
      ...
      const w = await terminal.run(`echo "payload:42" > ${tmpFile}`);
      expect(w).toSucceed();
      const r = await terminal.run(`cat ${tmpFile}`);
      expect(r).toSucceed();
      expect(r).toContainInOutput('payload:42');
      const c = await terminal.run(`rm -f ${tmpFile}`);
      expect(c).toSucceed();
