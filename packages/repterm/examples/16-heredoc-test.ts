...
describe('Heredoc (non-recording)', { record: true }, () => {
  test('multiline heredoc produces correct output', async ({ terminal }) => {
    // command 字符串包含 \n → 在非录制模式下走 session.write(command + '\n')
    // 字节直接写入 PTY → readline 逐行处理 → heredoc reader 从 PTY buffer 读取 body → 正常
    const result = await terminal.run('cat << \'EOF\'\nHello World\nLine 2\nEOF');
  ...
  test('heredoc with variable expansion', async ({ terminal }) => {
    const result = await terminal.run('NAME=Test; cat << EOF\nHello $NAME\nEOF');
    expect(result).toSucceed();
...
describe('Heredoc (recording)', { record: true }, () => {
  test('multiline heredoc — will fail due to Bracketed Paste', async ({ terminal }) => {
  ...
  test('single-line command — works fine in recording', async ({ terminal }) => {
