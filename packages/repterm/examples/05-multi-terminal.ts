...
describe('多终端测试', () => {
  test('创建第二个终端', async ({ $, terminal }) => {
    const result1 = await $`echo "Terminal 1"`;
    ...
    const result2 = await terminal2.$`echo "Terminal 2"`;
  ...
  test('使用文件进行进程间通信', async ({ $, terminal }) => {
    ...
    try {
      await $`echo "Hello from T1" > ${commFile}`;

      const readResult = await terminal2.$`cat ${commFile}`;
      expect(readResult).toHaveStdout('Hello from T1');

      await terminal2.$`echo "Response from T2" >> ${commFile}`;

      const fullResult = await $`cat ${commFile}`;
