...
describe('用户管理功能', () => {
  describe('用户注册', () => {
    test('成功注册新用户', async ({ terminal }) => {
      await step('准备测试数据', async () => {
        console.log('    Preparing test data...');
      });

      await step('执行注册命令', async () => {
        const result = await terminal.run('echo "User registered: alice"');
        expect(result).toSucceed();
      });

      await step('验证注册结果', async () => {
        const result = await terminal.run('echo "User exists: true"');
        expect(result).toHaveStdout('true');
      });
    });

    test('注册失败：用户已存在', async ({ terminal }) => {
      const result = await terminal.run('echo "Error: User already exists" >&2 && exit 1');
  ...
  describe('用户登录', () => {
    test('成功登录', async ({ terminal }) => {
      await step('发送登录请求', async () => {
        const result = await terminal.run('echo "Login successful"');
        expect(result).toSucceed();
      });
    });
  });
...
describe('测试步骤详细示例', () => {
  test('带有多个步骤的完整测试流程', async ({ terminal }) => {
    const setupResult = await step('环境准备', async () => {
      const result = await terminal.run('echo "Environment ready"');
      expect(result).toSucceed();
      return result;
    });

    const mainResult = await step('执行主要操作', async () => {
      const result = await terminal.run('echo "Operation completed"');
      expect(result).toSucceed();
      return result;
    });
  ...
  test('步骤可以返回值', async ({ terminal }) => {
    const version = await step('获取版本号', async () => {
      const result = await terminal.run('echo "1.2.3"');
