...
describe('Terminal 断言', () => {
  test('检查终端包含特定文本', async ({ terminal }) => {
    await terminal.run('echo "Welcome to Repterm"');
...
describe('snapshot 方法', () => {
  test('获取终端快照', async ({ terminal }) => {
    await terminal.run('echo "Line 1"');
    await terminal.run('echo "Line 2"');
