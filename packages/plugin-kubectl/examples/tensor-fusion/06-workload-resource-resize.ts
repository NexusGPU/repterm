      ...
      await step(
        ...
        async () => {
          const worker = await waitFor(
            async () => {
              workerPodName = await getWorkerPodName(kubectl, WORKLOAD_NAME);
              return getWorkerResources(kubectl, workerPodName);
            },
            (w) => w.tflops === TARGET_TFLOPS && w.vram === TARGET_VRAM,
            { description: 'worker annotations update to target values' }
          );
          afterWorkerTflops = worker.tflops;
          afterWorkerVram = worker.vram;
